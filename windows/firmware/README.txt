...
To perform the firmware update you MUST ensure that Windows allows the dock to be in DFU mode during the process. In practical term this means that the device needs to be recognonised and for this to occur an ad hoc driver must exist in the local driver store.
...
Run the <update.bat> executable script and follow the on-screen instructions to perform the firmware update. The firmware should then have been written to the device, but if you see a message to the extend that dfu-programmer found no device then it probably means you weren't quick enough to catch the dock in DFU mode - just try again!
